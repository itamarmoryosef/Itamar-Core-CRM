...
/** Admin-triggered; no `status` / `reminders_enabled` gate (those apply to cron auto batch only). */
function buildDocumentRequestMessage(
  fullName: string,
  portalLink: string
): string {
  const name = fullName.trim() || "לקוח";
  return `שלום ${name}, על מנת לקדם את התיק נא לצרף את המסמכים בקישור המצורף: ${portalLink}`;
}
...
export async function POST(request: Request) {
  ...
  if (fetchErr) {
    console.error(
      "[whatsapp/send-document-request] Supabase error",
      fetchErr.message
    );
  ...
  const message = buildDocumentRequestMessage(
  ...
  const ok = await sendWhatsAppTextMessage({
    ...
    text: message,
    logLabel: "whatsapp/send-document-request",
    logMeta: { clientId: client.id as string },
  });

  if (!ok) {
    return NextResponse.json(
      { error: "שליחת WhatsApp נכשלה. בדקו את ה-Bridge וההגדרות בשרת." },
      { status: 502 }
    );
  }
  ...
  const { error: upErr } = await supabase
    .from("clients")
    .update({ upload_request_active: true })
    .eq("id", clientId.trim());

  if (upErr) {
    return NextResponse.json(
      {
        error: `הודעה נשלחה אך עדכון המסד נכשל: ${upErr.message}. הפעילו שוב את הכפתור.`,
      },
      { status: 500 }
    );
  }
